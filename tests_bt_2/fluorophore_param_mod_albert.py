...
"""
Y'EST OÙ LE SCRIPTE ORIGINALE QUI FAISAIT ÇA PI QUI SAVAIT TOUT DANS UN GROS DICT ???
"""
    ...
    for pex_idx, p_ex in enumerate(p_ex_values):
        for psted_idx, p_sted in enumerate(p_sted_values):
            ...
            for i in range(n_repetitions):
                ...
                # WHAT NUMBER OF MOLECULES DO I NEED TO USE WITH ALBERT'S PARAMS? 5 - 140 seems ok?
...
#     if not os.path.exists(save_dir):
#         os.mkdir(save_dir)
save_dir = fluo_params_save_dir + "/fluo_alb"
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
